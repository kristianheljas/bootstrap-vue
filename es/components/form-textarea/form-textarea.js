...
export default {
  ...
  computed: {
    ...
    computedHeight: function computedHeight() /* istanbul ignore next: can't test getComputedProperties */{
      ...
      var offset = (parseFloat(computedStyle.borderTopWidth) || 0) + (parseFloat(computedStyle.borderBottomWidth) || 0) + (parseFloat(computedStyle.paddingTop) || 0) + (parseFloat(computedStyle.paddingBottom) || 0);
      // Calculate content height in "rows"
      var contentRows = (el.scrollHeight - offset) / lineHeight;
      ...
      var height = Math.max(Math.ceil(rows * lineHeight + offset), minHeight);
